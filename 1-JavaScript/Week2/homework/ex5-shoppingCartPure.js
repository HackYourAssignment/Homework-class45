...
------------------------------------------------------------------------------*/
// ! Function under test
function addToShoppingCart(shoppingCart, addItem) {
  const newCart = [...shoppingCart, addItem];
  // return newCart > 3 ? newCart.shift() : newCart;

  if (newCart.length > 3) {
    newCart.shift(); // Remove the first item
  }
  return newCart;
}
const shoppingCart = ['bananas', 'milk'];
console.log(addToShoppingCart(shoppingCart, 'foodTest'));
