...
// ! Function under test
function sanitizeFruitBasket(fruits, allergicFruit) {
  const sanitizeFruitBasket = fruits.filter((fruit) => fruit !== allergicFruit);
  return sanitizeFruitBasket;
}
...
describe('sanitizeFruitBasket', () => {
  ...
  test('should not modify the original `fruitBasket` array', () => {
    ...
    expect(originalFruitBasketContents.length === fruitBasket.length).toBe(
      true
    );
    expect(
      originalFruitBasketContents.indexOf('lemon') ===
        fruitBasket.indexOf('lemon')
    ).toBe(true);
  });
